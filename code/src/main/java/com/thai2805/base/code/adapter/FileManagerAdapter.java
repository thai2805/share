...
public interface FileManagerAdapter {
    ...
    @GetMapping(path = "${internal.filemanager.endpoint.getInfoS3:}", produces = {MediaType.APPLICATION_JSON_VALUE})
    String getInfoS3(@RequestParam String fileName, @RequestHeader Map<String, String> headers);

    @GetMapping(path = "${internal.filemanager.endpoint.getUrlS3:}", produces = {MediaType.APPLICATION_JSON_VALUE})
